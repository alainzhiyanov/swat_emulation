...
class RawWaterTank(Tank):
    ...
    def Plant(self, php, datafile):
        ...
        # Check value of P1.P101.DI_Run sent by first SubProcess
        try:
            with open('./P1-P101-DI_Run.txt', 'r') as file:
                # Read the first line and strip any whitespace
                value = file.readline().strip()
                # Convert the string to an integer
                p1_p101_di_run = float(value)
        except IOError:
            print("Error: Could not read file {}".format(file_path))
        ...
        if int(php.logdf.loc['P2.MV201.DI_ZSO','Value']) == 1 and int(p1_p101_di_run) == 1:
            self.h_t301=self.h_t301+self.p['f_mv201'] / self.p['S_t301']
    ...
    def main_loop(self):
        ...
        datafile = open('TankLevelReadings_x.csv','a')
        ...
        try:
            ...
            print("Server started in the background with PID: {}".format(process.pid))
            # return process
        ...
        while(True):
            if self.processctr == 0:
                logname = './log_'+str(self.processctr)+'_x.csv'
            else:
                logname = './log_'+str(self.processctr)+'_x.pkl'
            ...
            newlogname = './log_'+str(self.processctr)+'_plcx.pkl'
            self.logdf.to_pickle(newlogname)
            self.logdf = pd.DataFrame()
            if self.processctr > 0:
                histlogname = './historian/log_'+str(self.processctr)+'_plcx.pkl'
