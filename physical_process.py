...
class RawWaterTank(Tank):
    ...
    def main_loop(self):
        ...
        datafile = open('TankLevelReadings.csv','a')
        datafile.write("Time,Tank1,Tank3,Tank4\n")
        while(True):
            if self.processctr == 0:
                logname = './log_'+str(self.processctr)+'.csv'
            else:
                logname = './log_'+str(self.processctr)+'.pkl'
            ...
            newlogname = './log_'+str(self.processctr)+'_a.pkl'
            self.logdf.to_pickle(newlogname)
            self.logdf = pd.DataFrame()
            if self.processctr > 0:
                histlogname = './historian/log_'+str(self.processctr)+'.pkl'
